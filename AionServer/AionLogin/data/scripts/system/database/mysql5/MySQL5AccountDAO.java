...
public class MySQL5AccountDAO extends AccountDAO {
	...
	public Account getAccount(String name) {
		...
		try {
			...
			if (rs.next()) {
				...
				account.setMembershipExp(rs.getLong("membership_exp"));
	...
	public boolean insertAccount(Account account) {
		...
		PreparedStatement st = DB
			.prepareStatement("INSERT INTO account_data(`name`, `password`, access_level, membership_exp, activated, last_server, last_ip, last_mac, ip_force, toll) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");

		try {
			st.setString(1, account.getName());
			st.setString(2, account.getPasswordHash());
			st.setByte(3, account.getAccessLevel());
			st.setLong(4, account.getMembershipExp());
	...
	public boolean updateAccount(Account account) {
		int result = 0;
		PreparedStatement st = DB.prepareStatement("UPDATE account_data SET `name` = ?, `password` = ?, access_level = ?, membership_exp = ?, last_server = ?, last_ip = ?, last_mac = ?, ip_force = ? WHERE `id` = ?");
	...
	public boolean updateMembership(final int accountId) {
		return DB.insertUpdate(
			"UPDATE account_data SET membership_exp = old_membership, expire = NULL WHERE id = ? and expire < CURRENT_TIMESTAMP",
			new IUStH() {
	...
	/* (non-Javadoc)
	 * @see com.aionemu.loginserver.dao.AccountDAO#updataBan(int, byte)
	 */
	@Override
	public boolean updataBan(final int accountid, final byte banid) {
		return DB.insertUpdate("UPDATE account_data SET activated = ? WHERE id = ?", new IUStH() {

			@Override
			public void handleInsertUpdate(PreparedStatement preparedStatement) throws SQLException {
				preparedStatement.setByte(1, banid);
				preparedStatement.setInt(2, accountid);
				preparedStatement.execute();
			}
		});
