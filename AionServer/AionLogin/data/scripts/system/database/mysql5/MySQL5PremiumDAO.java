...
public class MySQL5PremiumDAO extends PremiumDAO {
	...
	public int getAccountCode(int accountId) {
		int accountCode = 0;
		PreparedStatement st = DB.prepareStatement("SELECT code FROM account_data WHERE id=?");
		try {
			st.setInt(1, accountId);
			ResultSet rs = st.executeQuery();
			if (rs.next()) {
				accountCode = rs.getInt("code");
			}
		}
		catch (Exception e) {
			log.error("getAccountCode [select code] "+accountId, e);
		}
		finally {
			DB.close(st);
		}
